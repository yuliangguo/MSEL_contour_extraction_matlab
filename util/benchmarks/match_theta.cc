    ...
    Matrix& m1, Matrix& m2)
{
    ...
    assert (theta1.nrows() == theta2.nrows());
    assert (theta1.ncols() == theta2.ncols());
    assert (theta_diff >= 0 );
    assert (maxDist >= 0);
        ...
        for (int x1 = 0; x1 < width; x1++) {
            ...
            for (int v = -r; v <= r; v++) {
                for (int u = -r; u <= r; u++) {
                    ...
                    if ( theta1(y1,x1) >= -M_PI && theta1(y1,x1) <= M_PI  )
                    {
                        double theta_bmap1=theta1(y1,x1);
                        double theta_bmap2=theta2(y2,x2);
                        ...
                        double theta_angle = std::min(
                            fabs(theta_bmap1-theta_bmap2),
                            fabs(theta_bmap1-theta_bmap2-M_PI));
