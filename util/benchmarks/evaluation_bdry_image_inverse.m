function [thresh,cntR,sumR,cntP,sumP] = evaluation_bdry_image_inverse(inFile,gtFile, prFile, nthresh, maxDist, thinpb)
...
for t = 1:nthresh,
    ...
    accR = zeros(size(bmap));
    
    BGT_all_YGTs = zeros(size(bmap,1),size(bmap,2), 3);
    ...
    for i = 1:numel(groundTruth),
        ...
        accR = accR | match2;
        % accumulate all ground truth
        gt = gt | groundTruth{i}.Boundaries;
        % compute recall
        sumR(t) = sumR(t) + sum(groundTruth{i}.Boundaries(:));
        cntR(t) = cntR(t) + sum(match2(:)>0);
        
%         BGT_all_YGTs(:,:,i) = BGT_all_YGTs(:,:,i)+match1;
    ...
    eval_image = zeros(size(bmap,1),size(bmap,2), 3);
    eval_image(:,:,1) = eval_image(:,:,1)+miss;
    eval_image(:,:,2) = eval_image(:,:,2)+fp;
    
    eval_image(:,:,1) = eval_image(:,:,1)+accP+accR;
    eval_image(:,:,2) = eval_image(:,:,2)+accP+accR;
    eval_image(:,:,3) = eval_image(:,:,3)+accP+accR;
    
    imshow(eval_image, 'border', 'tight');
    h1=figure(1);
    set(h1, 'PaperPositionMode','auto')
%      print(h1,'-djpeg95','-r0',[inFile(1:end-4),'_match.jpg']);
end
