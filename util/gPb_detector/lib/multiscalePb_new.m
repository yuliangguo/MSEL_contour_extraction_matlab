function [mPb_nmax, mPb_nmax_rsz, bg1, bg2, bg3, cga1, cga2, cga3, cgb1, cgb2, cgb3, tg1, tg2, tg3, textons] = multiscalePb_new(im, rsz, beta)
...
% this weights is for logistic combination rather than linear combination
weights = beta;
if(size(im,3)==1)
    im(:,:,2)=im(:,:,1);im(:,:,3)=im(:,:,1);
end
...
for o = 1 : size(mPb_all, 3),
    ...
    mPb_all(:, :, o) = 1 ./ (1 + (exp(-(ones(size(l1))*weights(1) + l1 + a1 + b1 + t1 + l2 + a2 + b2 + t2 + l3 + a3 + b3 + t3))));
...
if rsz < 1,
    ...
    for o = 1 : size(mPb_all, 3),
        ...
        mPb_all(:, :, o) = imresize(1 ./ (1 + (exp(-(ones(size(l1))*weights(1) + l1 + a1 + b1 + t1 + l2 + a2 + b2 + t2 + l3 + a3 + b3 + t3)))), rsz);
