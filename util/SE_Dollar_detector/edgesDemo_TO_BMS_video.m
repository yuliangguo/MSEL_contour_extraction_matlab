function edgesDemo_TO_BMS_video(src_path, start_frame, end_frame)
...
th = 0.03;
% src_path = '/gpfs/scratch/yg13/cornell/101815M1-Social/img/';
% img_files = dir([src_path '*.png']);
% out_path = 'results/CUB14/';
% out_path = '/gpfs/scratch/yg13/cornell/101815M1-Social/edge_org/';
out_path = [src_path(1:end-4) 'edge_org/'];
mkdir(out_path);
for i = start_frame:end_frame
    i
    imgFile = [src_path num2str(i, '%04d') '.png'];
...
%     imshow(edgemap)
    ...
    dst_edg_file = [out_path num2str(i, '%04d') edg_ext];
    dst_ori_file = [out_path num2str(i, '%04d') orient_ext];
    ...
    save_edg([out_path num2str(i, '%04d') '.edg'], TO_edges, [size(I,2) size(I,1)]);
