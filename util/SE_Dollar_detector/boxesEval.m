...
parfor i=1:M*T*K 
    ...
    disp(['i=' num2str(i) '/' num2str(M*T*K)]);
...
for type=1:2
  ...
  h0 = figure(f); f=f+1; clf; hold on; hs=zeros(M,K);
  ...
  save([o.resDir 'plots/' s{type} '-' o.data.split '-' o.fName '.mat'], 'xs', 'R')
  % save curve images
  % savefig([o.resDir 'plots/' s{type} '-' o.data.split '-' o.fName],'png');
  set(h0, 'PaperPositionMode','auto')
  print(h0,'-djpeg95','-r0',[o.resDir 'plots/' s{type} '-' o.data.split '-' o.fName '.png']);
  % -------------------------------------
end
end
