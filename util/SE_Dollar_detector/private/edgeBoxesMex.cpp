...
public:
  ...
  void generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, arrayf &boxImg);
...
void EdgeBoxGenerator::generate( Boxes &boxes, arrayf &E, arrayf &O, arrayf &V, arrayf &boxImg )
{
  clusterEdges(E,O,V); prepDataStructs(E); scoreAllBoxes(boxes);
    Box testbox;
    testbox.c = (E._w)/4; testbox.r = (E._h)/4; testbox.w = (E._w)/2; testbox.h = (E._h)/2; testbox.s=1;
    drawBox( testbox, E, boxImg );
}
...
void EdgeBoxGenerator::clusterEdges( arrayf &E, arrayf &O, arrayf &V )
{
  ...
  for( c=rad; c<w-rad; c++ ) for( r=rad; r<h-rad; r++ ) {
    ...
    for( cd=-rad; cd<=rad; cd++ ) for( rd=-rad; rd<=rad; rd++ ) {
      ...
      if( found ) continue;
      float o=atan2(meanY[s0]-meanY[s1],meanX[s0]-meanX[s1]);
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(nl > 3) mexErrMsgTxt("At most three outputs expected.");
  ...
  // optionally create memory for visualization
    ...
    arrayf boxImg; if( nl>2 ) {
        pl[2] = mxCreateNumericArray(3,ds,mxSINGLE_CLASS,mxREAL);
        boxImg._x = (float*) mxGetData(pl[2]); boxImg._h=h; boxImg._w=w;
    }
  ...
  edgeBoxGen.generate( boxes, E, O, V, boxImg );
